...
def model_synmetric_resize_fn(shape=(32,32,2), 
                              nblocks=5, base_dim=4, nstack_layer=3, conv_kernel_size=5,
  ...
  def kernel_adjust(kernel_size, oheight, iblock):
    """ fucntion to adjust kernel size 
    """
    if oheight/(2**iblock) >= kernel_size:
      return kernel_size        
    else:
      for pk in [7,5,3,1]:
        if oheight/(2**iblock) > pk:
          return pk
  ...
  x = Conv2D(filters=params["filters"][0], kernel_size=kernel_size, padding='same', kernel_initializer='he_normal')(x)
  x = LeakyReLU()(x)
  for iblock in range(nblocks):
    filters = params["filters"][iblock]
    kernel_size = kernel_adjust(params["kernel_size"], oheight, iblock)
    ...
    if iblock < nblocks-1:
      ...
      kernel_size = kernel_adjust(params["kernel_size"], oheight, iblock+1)
